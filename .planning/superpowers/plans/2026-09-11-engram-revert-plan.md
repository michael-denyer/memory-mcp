...
PR-5, realised on 2026-09-12. Trimming 33 tools left `commands/test-all.md`, `commands/resources/testing/`, `skills/memory-mcp/SKILL.md`, and `CONTRIBUTING.md` pointing at removed tools and a removed extra, and the `embedding_backend` setting became dead. The root widened PR-5 to those files, deleted the manual QA command set, and allowed the plan's orphan-method deletion for the 16 storage methods with no caller in `src/`. PR-1 and PR-5 both touch `embeddings.py`; at rebase the MLX provider goes and PR-1's `embedding_device` setting and `test-macos` job stay. Outside the program: `/memory-mcp:recall` in balanced mode gates out a stored memory for a two-word query on every head, so the short-query recall threshold needs its own change.
